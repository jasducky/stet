...
(function () {
  ...
  function wireSelection() {
    document.addEventListener("mouseup", function (e) {
      if (editing) return;
      if (e.target.closest && e.target.closest(".rv-bar,.rv-side,.rv-tools,.rv-modal")) return;
  ...
  function thread(c) {
    ...
    if (c.proposal) {
      prop = '<div class="rv-prop"><div class="rv-prop-h">Proposed change</div>' +
             ...
             '<button data-a="reject">Needs changes</button></div></div>';
    }
  ...
  function boot() {
    ...
    status(CFG.units.length + " editable regions" +
